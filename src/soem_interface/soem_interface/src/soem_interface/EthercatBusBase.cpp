...
namespace soem_interface
{
  ...
  struct EthercatBusBaseTemplateAdapter::EthercatSlaveBaseImpl
  {
    ...
    bool busIsAvailable() const { return soem_interface::busIsAvailable(name_); }
    ...
    bool addSlave(const EthercatSlaveBasePtr &slave)
    {
      ...
      std::sort(slaves_.begin(), slaves_.end(),
                [](const EthercatSlaveBasePtr &a, const EthercatSlaveBasePtr &b) -> bool
    ...
    bool startup(std::atomic<bool> &abortFlag, const bool sizeCheck, int maxDiscoverRetries)
    {
...

      {
        ...
        for (int retry = 0; retry <= maxDiscoverRetries; retry++)
        {
          if (abortFlag)
          {
            MELO_WARN_STREAM("[soem_interface::" << name_ << "] "
          ...
          {
            // on some of the older anydrives there seems to be a short race between bus is responsive and slave is fully ready...
            // so give them this 1 sec to be fully ready to be started...
            soem_interface::threadSleep(1.0);
            break;
          }
          if (retry == maxDiscoverRetries)
          {
            MELO_ERROR_STREAM("[soem_interface::" << name_ << "] "
          ...
          MELO_INFO_STREAM("[soem_interface::" << name_ << "] No slaves have been found, retrying " << retry + 1 << "/"
                                               << maxDiscoverRetries << " ...");
        }
        ...
        MELO_INFO_STREAM("[soem_interface::" << name_ << "] The following " << nSlaves << " slaves have been found and configured:");
        for (int slave = 1; slave <= nSlaves; slave++)
        {
          MELO_INFO_STREAM("[soem_interface::" << name_ << "] Address: " << slave << " - Name: '"
      ...
      {
        MELO_INFO_STREAM("[soem_interface::" << name_ << "] Starting slave: " << slave->getName())
        if (!slave->startup())
        {
          MELO_ERROR_STREAM("[soem_interface::" << name_ << "] Slave '" << slave->getName() << "' was not initialized successfully.");
          return false;
        }
        else
        {
          MELO_DEBUG_STREAM("[soem_interface::" << name_ << "] Successfully started slave: " << slave->getName())
        }
      }
      ...
      MELO_DEBUG_STREAM("[soem_interface::" << name_ << "] Configured ioMap with size: " << ioMapSize)
    ...
    void updateRead()
    {
      ...
      if (wkc_ < expectedWorkingCounter)
      {
        ...
        MELO_DEBUG_STREAM("[soem_interface::" << name_ << "] Working counter too low counter: " << workingCounterTooLowCounter_)
        MELO_DEBUG_THROTTLE_STREAM(1.0, "[soem_interface::" << getName() << "] Update Read:" << this);
        MELO_WARN_STREAM("[soem_interface::" << name_ << "] Working counter is too low: " << wkc_.load() << " < "
                                             << expectedWorkingCounter << ", wkc's to low in a row: " << workingCounterTooLowCounter_);
        {
          std::lock_guard<std::mutex> guard(contextMutex_);
          MELO_WARN_STREAM("[soem_interface" << name_ << "] For all slaves alStatusCode: 0x" << std::setfill('0') << std::setw(8)
        ...
        {
          MELO_ERROR_THROTTLE_STREAM(1.0, "[soem_interface" << name_ << "] Bus is not ok. Too many working counter too low in a row: "
                                                            << workingCounterTooLowCounter_)
        }
        return;
      }
    ...
    void updateWrite()
    {
      if (sentProcessData_)
      {
        MELO_DEBUG_STREAM("[soem_interface] Sending new process data without reading the previous one.");
      }
    ...
    void shutdown()
    {
      ...
      {
        MELO_INFO_STREAM("[soem_interface::" << name_ << "] Closing socket ...");
        ecx_close(&ecatContext_);
    ...
    bool doBusMonitoring(bool logErrorCounterForDiagnosis)
    {
      ...
      if (busDiagState_ == BusDiagState::StateReading)
      {
        ...
        // possible.
        std::lock_guard<std::mutex> guard(contextMutex_);
        if ((lowestSlaveState & 0x0f) < EC_STATE_OPERATIONAL)
        { // if ECAT Error bus state is e.g. 0x14 = 0x10 (error) + 0x04 (safeOP)
          ...
          for (const auto &slave : slaves_)
          {
            ...
            {
              MELO_INFO_STREAM("[soem_interface::" << name_ << "] Slave: " << slave->getName() << " alStatusCode: 0x" << std::setfill('0')
              ...
              if (ecatContext_.slavelist[slave->getAddress()].state == EC_STATE_NONE && !ecatContext_.slavelist[slave->getAddress()].islost)
              {
                ...
                // todo  Trying to recover the lost slave. !NOT IMPLEMENTED! example: in soem simple_test.c
      ...
      if (busDiagState_ == BusDiagState::CounterReading)
      {
        ...
        {
          MELO_WARN_STREAM(
              "[soem_interface::BusMonitoring::" << name_ << "] Could not read Error counters for slave: " << selectedSlave->getName())
        }
    ...
    bool sdoWrite(const uint16_t slave, const uint16_t index, const uint8_t subindex, const bool completeAccess, int size, void *buf)
    {
      ...
      if (wkc <= 0)
      {
        ...
        if (slave == 0)
        {
          MELO_INFO_STREAM("[soem_interface::" << name_ << "] Worst AL status code of all slaves, alStatusCode: 0x" << std::setfill('0')
        ...
        {
          MELO_INFO_STREAM("[soem_interface::" << name_ << "] Slave: " << slaves_[slave - 1]->getName() << " alStatusCode: 0x"
  ...
  private:
    ...
    void setStateLocked(const uint16_t state, const uint16_t slave = 0)
    {
      if (!initlialized_)
      {
        MELO_WARN_STREAM("[soem_interface::" << name_ << "] Bus " << name_ << " was not successfully initialized, skipping operation");
        return;
      }
      ...
      if (slave == 0)
      {
        MELO_DEBUG_STREAM("[soem_interface::" << name_ << "] All slaves on State " << EthercatBusBase::getStateString(state)
                                              << " has been set.");
      }
      else
      {
        MELO_DEBUG_STREAM("[soem_interface::" << name_ << "] Slave " << slaves_[slave - 1]->getName() << " State "
    ...
    bool waitForStateLocked(const uint16_t state, const uint16_t slave = 0, const unsigned int maxRetries = 20)
    {
      ...
      {
        MELO_INFO_STREAM("[soem_interface::" << name_ << "] Slave: " << slave << ": State " << EthercatBusBase::getStateString(state)
      ...
      MELO_WARN_STREAM("[soem_interface::" << name_ << "] Slave " << slave << ": Targetstate " << EthercatBusBase::getStateString(state)
  ...
  {
    return soem_interface::busIsAvailable(name);
  }
  ...
  void EthercatBusBase::setState(soem_interface::ETHERCAT_SM_STATE state, const uint16_t slave)
  {
  ...
  bool EthercatBusBase::waitForState(soem_interface::ETHERCAT_SM_STATE state, const uint16_t slave, const unsigned int maxRetries)
  {
  ...
  soem_interface::ETHERCAT_SM_STATE EthercatBusBase::getEthercatState(const uint16_t slave)
  {
