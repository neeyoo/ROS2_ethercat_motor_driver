...
#include <soem_interface_example/ExampleSlave.hpp>
...
int main(int argc, char **argv)
{
  ...
  std::unique_ptr<soem_interface::EthercatBusBase> bus = std::make_unique<soem_interface::EthercatBusBase>(busName);

  std::shared_ptr<soem_interface_example::ExampleSlave> slave =
      std::make_shared<soem_interface_example::ExampleSlave>(slaveName, bus.get(), slaveAddress);
